...
class TSWriter {
    ...
    func writeSampleBuffer(_ PID:UInt16, streamID:UInt8, sampleBuffer:CMSampleBuffer) {
        ...
        try self.currentFileHandle?.write(Data(bytes: UnsafePointer<UInt8>(bytes), count: bytes.count))
    }
    ...
    func rorateFileHandle(_ timestamp:CMTime) -> Bool {
        ...
        try self.currentFileHandle?.write(Data(bytes: bytes, count: bytes.count))
